...
{
	int lvl; ///low three
	int right_key, left_key, m_Count; /// right and left key and count
	int *top; /// array 
	bool full; /// proverka, pustoe ili net.
public:
	...
	void Fill(int lv, int max_count) /// Zapolnenie tree
	{
		full = false;
	...
	int Check() /// proverka ryada na polnitu
	{
		if (full)
		{
			return -1;
	...
	int Check1() /// proverka ryada na pustotu
	{
		if (m_Count != 0)
			return m_Count;
		else
	...
	void Add(int Element, int lv, int kol_max, int position) /// add element on right or new low (uroven)
	{
		full = false;
		lvl = lv;
	...
	void Print(int lvl_num) ///vivodim derevo
	{
		int k1 = lvl_num - lvl;
		k1 = pow(2.0, k1);
...
int main()
{
	...
	Tree *A;
	Tree *B;
	B = new Tree[lvl_number]; /// videlaem pamat
